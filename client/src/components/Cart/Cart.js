...
const Cart = () => {
  ...
  //   const [axistingDetails,setAxistingDetails] =useState()

  //   const axistingDetails = {
  //     userId: profile?._id,
  //     products: checkOuteProducts,
  //     email: profile?.email,
  //     customer: profile?.name,
  //     price: grandTotal,
  //     categories: productsCategoris,
  //   };
  //   console.log({ axistingDetails });
  ...
  useEffect(() => {
    ...
    const totals =
      parseInt(
        CART.reduce(
          (total, prd) => total + prd.oldPrice * prd.pdCount,
          0
        ).toFixed(2)
      ) || 0;
    //! =====> VAT/Tax
    ...
    setTax(taxx);
    setTotal(totals);
    setgrandTotal((totals + Number(taxx) + delivery).toFixed(2));
  }, [CART, total]);
  //! =====> Remove product
