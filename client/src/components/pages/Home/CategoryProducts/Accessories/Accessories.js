...
const Accessories = () => {
  ...
  const cats = [
    ...new Set(product?.filter((card) => card.category === "Accessories")),
  ];
  ...
  const filtered = () => {
    const category = product?.filter(
      (cat) => cat?.category.toLowerCase() === "Accessories".toLowerCase()
    );
        ...
        <div className="w-full z-1">
          <div className="price-range-shadow mb-5 py-2 ">
            <div className="flex gap-5 md:gap-10 items-center md:justify-between my-5 justify-end">
              <div>
                <h1 className="font-bold text-gray-500  px-5">
                  {" "}
          ...
          {instock?.length >= 1 ? (
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-10  price-range-shadow p-10 z-20">
              {instock?.map((product) => {
                return (
                  <ProductsDetails
                    key={product._id}
