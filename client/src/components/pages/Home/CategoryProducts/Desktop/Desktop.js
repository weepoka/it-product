...
const CategoryProducts = () => {
	...
	const cats = [
		...new Set(product?.filter((card) => card.category === 'Desktop')),
	];

	useEffect(() => {
		getProductsByCategory(setSelectedProduct, id);
		// filtered();
	}, [id]);
	const filtered = () => {
		const category = product?.filter(
			(cat) =>
				cat.category.toLocaleLowerCase() === 'Desktop'.toLocaleLowerCase()
		);
		...
		<div className='max-w-screen-2xl  mx-auto py-5 '>
			<div className='md:px-10 flex items-center px-2 price-range-shadow py-5 mb-5 '>
				...
				{/* <p className='text-blue-500 md:ml-2 font-bold  md:text-xl'>
					<NavLink to='/'> {cats[0]?.category}</NavLink>
				</p> */}
			</div>
			<div
				className='
